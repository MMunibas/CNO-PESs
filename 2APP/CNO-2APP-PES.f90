!################################################################
!       Potential energy surface for the 2A'' state of CNO      #
!################################################################
!This subroutines use the RKHS.f90 fortran file for the RKHS interpolation
!and pes21.csv, pes22.csv, pes23.csv and asymp2.dat data files
!
!Call the full triatomic surface with
!
!call ocn2apppes(tmpr, totener, dvdr)
!input
...
real*8, parameter :: wtol = epsilon(1.0d0), dr1 = 0.48d0, dr2 = 0.40d0, dr3 = 0.28d0
...
if (.not. ker1) then
  inquire(file="../pes21.kernel", exist=ker1)   ! file_exists will be true if the file exists and false otherwise
end if

if (.not. ker2) then
  inquire(file="../pes22.kernel", exist=ker2)   ! file_exists will be true if the file exists and false otherwise
end if

if (.not. ker3) then
  inquire(file="../pes23.kernel", exist=ker3)   ! file_exists will be true if the file exists and false otherwise
end if
...
open(unit=1001,file="../asymp2.dat", status = "old")
...
if (.not. kread) then
  if (ker1 .and. ker2 .and. ker3 ) then
    call pes21%load_from_file("../pes21.kernel")
    call pes22%load_from_file("../pes22.kernel")
    call pes23%load_from_file("../pes23.kernel")
    kread = .true.
  else
    call pes21%read_grid("../pes21.csv")
!    print*,"IAMHERE"
...
!    call pes21%calculate_coefficients_slow(lambda)
    ...
    call pes21%save_to_file("../pes21.kernel")
!
    call pes22%read_grid("../pes22.csv")
!    print*,"IAMHERE"
...
!    call pes22%calculate_coefficients_slow(lambda)
    ...
    call pes22%save_to_file("../pes22.kernel")

    call pes23%read_grid("../pes23.csv")
!    print*,"IAMHERE"
...
!    call pes23%calculate_coefficients_slow(lambda)
    ...
    call pes23%save_to_file("../pes23.kernel")
...
if (sno==3) then
  ...
  anener = 0.0d0
  ander=0.0d0
  call pes23%evaluate_fast(x,anener,ander)
...
subroutine ocn2apppes(tmpr, totener, dvdr)
implicit none
