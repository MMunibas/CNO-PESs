...
!and pes11.csv, pes12.csv, pes13.csv and asymp1.dat data files
!
!Call the full triatomic surface with
!
!call ocn2appes(tmpr, totener, dvdr)
!input
...
implicit none
real*8,parameter :: m1 = 12.0107d0, m2 = 14.0067d0, m3 = 15.9994d0
...
real*8, parameter :: wtol = epsilon(1.0d0), dr1 = 0.71d0, dr2 = 0.69d0, dr3 = 0.68d0
...
if (.not. ker1) then
  inquire(file="../pes11.kernel", exist=ker1)   ! file_exists will be true if the file exists and false otherwise
end if

if (.not. ker2) then
  inquire(file="../pes12.kernel", exist=ker2)   ! file_exists will be true if the file exists and false otherwise
end if

if (.not. ker3) then
  inquire(file="../pes13.kernel", exist=ker3)   ! file_exists will be true if the file exists and false otherwise
end if
...
open(unit=1001,file="../asymp1.dat", status = "old")
...
if (.not. kread) then
  if (ker1 .and. ker2 .and. ker3 ) then
    call pes11%load_from_file("../pes11.kernel")
    call pes12%load_from_file("../pes12.kernel")
    call pes13%load_from_file("../pes13.kernel")
    kread = .true.
  else
    call pes11%read_grid("../pes11.csv")
!    print*,"IAMHERE"
...
!    call pes11%calculate_coefficients_slow(lambda)
    ...
    call pes11%save_to_file("../pes11.kernel")
!
    call pes12%read_grid("../pes12.csv")
!    print*,"IAMHERE"
...
!    call pes12%calculate_coefficients_slow(lambda)
    ...
    call pes12%save_to_file("../pes12.kernel")

    call pes13%read_grid("../pes13.csv")
!    print*,"IAMHERE"
...
!    call pes13%calculate_coefficients_slow(lambda)
    ...
    call pes13%save_to_file("../pes13.kernel")
...
subroutine ocn2appes(tmpr, totener, dvdr)
implicit none
