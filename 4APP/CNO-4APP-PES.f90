!################################################################
!       Potential energy surface for the 4A'' state of CNO      #
!################################################################
!This subroutines use the RKHS.f90 fortran file for the RKHS interpolation
!and pes31.csv, pes32.csv, pes33.csv and asymp3.dat data files
!
!Call the full triatomic surface with
!
!call ocn4apppes(tmpr, totener, dvdr)
!input
!tmpr => an array with R(CN), R(NO) and R(CO) bond distances in bohr
!output
!totener => potential energy in hartree
!dvdr => Derivatives of the potential with respect to R(CN), R(NO)
! and R(CO) bond distances in hartree/bohr
!-----------------------------------------------------------------
!Call the individual diatomic potentials with 
!
!call diatcn(r,ener,der) ! for CN diatom
!call diatno(r,ener,der) ! for NO diatom
!call diatco(r,ener,der) ! for CO diatom
!
!input
!r => diatomic bond distance R in bohr
!output
!ener => potential energy in hartree
!der => Derivative of the potential with respect to R in hartree/bohr
...
implicit none
real*8,parameter :: m1 = 12.0d0, m2 = 14.003074d0, m3 = 15.99491462d0
...
real*8, parameter :: wtol = epsilon(1.0d0), dr1 = 1.06d0, dr2 = 0.80d0, dr3 = 1.0d0
...
integer :: kk, ii
type(kernel), save  :: pes31, pes32, pes33           ! The kernel type is needed to set up and evaluate a RKHS model
logical, save :: stored = .false., kread = .false.
logical, save :: ker1 = .false., ker2 = .false., ker3 = .false.

if (.not. ker1) then
  inquire(file="../pes31.kernel", exist=ker1)   ! file_exists will be true if the file exists and false otherwise
end if

if (.not. ker2) then
  inquire(file="../pes32.kernel", exist=ker2)   ! file_exists will be true if the file exists and false otherwise
end if

if (.not. ker3) then
  inquire(file="../pes33.kernel", exist=ker3)   ! file_exists will be true if the file exists and false otherwise
end if
...
open(unit=1001,file="../asymp3.dat", status = "old")
...
if (.not. kread) then
  if (ker1 .and. ker2 .and. ker3 ) then
    call pes31%load_from_file("../pes31.kernel")
    call pes32%load_from_file("../pes32.kernel")
    call pes33%load_from_file("../pes33.kernel")
    kread = .true.
  else
    call pes31%read_grid("../pes31.csv")
!    print*,"IAMHERE"
    call pes31%k1d(1)%init(TAYLOR_SPLINE_N2_KERNEL)         ! choose one-dimensional kernel for dimension 1
    call pes31%k1d(2)%init(RECIPROCAL_POWER_N2_M6_KERNEL)   ! choose one-dimensional kernel for dimension 2
    call pes31%k1d(3)%init(RECIPROCAL_POWER_N2_M6_KERNEL) 

!    call pes31%calculate_coefficients_slow(lambda)
    call pes31%calculate_coefficients_fast()

    call pes31%calculate_sums()

    call pes31%save_to_file("../pes31.kernel")
!
    call pes32%read_grid("../pes32.csv")
!    print*,"IAMHERE"
    call pes32%k1d(1)%init(TAYLOR_SPLINE_N2_KERNEL)         ! choose one-dimensional kernel for dimension 1
    call pes32%k1d(2)%init(RECIPROCAL_POWER_N2_M6_KERNEL)   ! choose one-dimensional kernel for dimension 2
    call pes32%k1d(3)%init(RECIPROCAL_POWER_N2_M6_KERNEL)

!    call pes32%calculate_coefficients_slow(lambda)
    call pes32%calculate_coefficients_fast()

    call pes32%calculate_sums()

    call pes32%save_to_file("../pes32.kernel")

    call pes33%read_grid("../pes33.csv")
!    print*,"IAMHERE"
    call pes33%k1d(1)%init(TAYLOR_SPLINE_N2_KERNEL)         ! choose one-dimensional kernel for dimension 1
    call pes33%k1d(2)%init(RECIPROCAL_POWER_N2_M6_KERNEL)   ! choose one-dimensional kernel for dimension 2
    call pes33%k1d(3)%init(RECIPROCAL_POWER_N2_M6_KERNEL)

!    call pes33%calculate_coefficients_slow(lambda)
    call pes33%calculate_coefficients_fast()

    call pes33%calculate_sums()

    call pes33%save_to_file("../pes33.kernel")
...
if (sno==1) then
  ...
  anener = 0.0d0
  ander=0.0d0
  call pes31%evaluate_fast(x,anener,ander)
...
if (sno==2) then
  ...
  anener = 0.0d0
  ander=0.0d0
  call pes32%evaluate_fast(x,anener,ander)
...
if (sno==3) then
  ...
  anener = 0.0d0
  ander=0.0d0
  call pes33%evaluate_fast(x,anener,ander)
...
subroutine ocn4apppes(tmpr, totener, dvdr)
implicit none
...
end subroutine ocn4apppes

subroutine diatcn(r,ener,der)
use rep_ker
implicit none
real*8, intent(in) :: r
...
der=0.0d0
do kk = 1, nda3
    ener = ener + drker26(r,darray3(kk,1))*darray3(kk,2)
    der = der + ddrker26(r,darray3(kk,1))*darray3(kk,2)
end do
